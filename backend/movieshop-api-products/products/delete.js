...
module.exports.delete = async (event, context, callback) => {
  ...
  try {
    ...
    console.log("22222");
    const instance = `/merchants/${event.pathParameters.id}/products/${event.pathParameters.idProduct}`;
    console.log("3333");
    if (JSON.stringify(result) === '{}') {
      ...
      console.log("aaaa");
      var theProduct = result.Item.products.filter(s => s.id === event.pathParameters.idProduct);
      console.log("bbbb");
      console.log(theProduct)
      console.log("ffff");

      if (result.Item.products.length === 1){
        ...
        const filteredProductList = result.Item.products.filter(s => s.id != event.pathParameters.idProduct);
        console.log("ffff11");
        if (filteredProductList.length == result.Item.products.length){
          console.log("ffff222");
        ...
        result.Item.products = filteredProductList;
        ...
        try{
          ...
          return;
            ...
            const error_lam = await getError("pt", "insert_generic_product_error",400,instance);
            callback(null, error_lam);
        }
      }
    }
