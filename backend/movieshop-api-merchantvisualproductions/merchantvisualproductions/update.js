...
const required_fields = ["id","name","visualproducts"]
...
async function updateMerchantVisualProduction(theMerchantVisualProduction,data,path){
  ...
  Object.entries(data).forEach(async ([key, val]) => {
      ...
      if (!exclusions.includes(key)){
        ...
        if (key === "complement"){
          theMerchantVisualProduction.address[key] = val
        } else {
  ...
  try{
    ...
    if (JSON.stringify(result) === '{}') {
      ...
      try{
        ...
        console.log("a9")
          const error_lam = await getError("en", "insert_generic_merchantvisualproduction_error",400,event.path,[]);
          console.log("a10")
          callback(null, error_lam);
      }
      const response = {
